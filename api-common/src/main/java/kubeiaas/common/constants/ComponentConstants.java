package kubeiaas.common.constants;

/**
 * Constants for KubeIaaS Components/Services
 */
public class ComponentConstants {
    public static final String IAAS_CORE = "iaas-core";
    public static final String IAAS_AGENT = "iaas-agent";
    public static final String DB_PROXY = "db-proxy";
    public static final String RESOURCE_OPERATOR = "resource-operator";
    public static final String IMAGE_OPERATOR = "image-operator";
    // ---
    public static final String DHCP_CONTROLLER = "dhcp-controller";
    public static final String VNC_CONTROLLER = "vnc-controller";
    public static final String IMAGE_CONTROLLER = "image-controller";
    public static final String VOLUME_CONTROLLER = "volume-controller";
    public static final String VM_CONTROLLER = "vm-controller";
    public static final String HOST_CONTROLLER = "host-controller";
    // ---
    public static final String DHCP = "DHCP";
    public static final String VNC = "noVNC";
    public static final String LIBVIRT = "Libvirt";
    // ---
    public static final String NFS = "NFS";
    public static final String NFS_IP = "NFS_IP";
    public static final String NFS_DIR = "NFS_DIR";

}
